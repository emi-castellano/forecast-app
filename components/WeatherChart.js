...
import { StyleSheet, View, ScrollView, Button, Text, Dimensions } from 'react-native';
...
export default class WeatherChart extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            data: mydata.list.map((item) => item.main.temp),
            xAxis: mydata.list.map((item) => item.dt_txt)
        }
    }

    changeShownData = (valueToShow) => {
        switch (valueToShow) {
            case 'temp':
                this.setState({ data: mydata.list.map((item) => item.main.temp) }, console.log(this.state.data.length))
                break;
            case 'humidity':
                this.setState({ data: mydata.list.map((item) => item.main.humidity) }, console.log(this.state.data.length))
                break;
            case 'pressure':
                this.setState({ data: mydata.list.map((item) => item.main.pressure) }, console.log(this.state.data.length))
                break;
            default:
                break;
        }
        ...
        return (
            <View style={styles.container}>
                ...
                <View style={styles.buttonsContainer}>
                    <Button title="Temperature" onPress={() => { this.changeShownData('temp') }} />
                    <Button title="Humidity" onPress={() => { this.changeShownData('humidity') }} />
                    <Button title="Pressure" onPress={() => { this.changeShownData('pressure') }} />
                </View>
                ...
                <ScrollView horizontal={true} style={styles.chartContainer}>
                    <YAxis
                        ...
                        svg={axesSvg}
                    />
                    <View style={{ flex: 1, marginLeft: 20, width: this.state.xAxis.length * 50 }}>
                        <LineChart
                            style={{ flex: 1 }}
                            data={this.state.data}
                            contentInset={verticalContentInset}
                        ...
                        <XAxis
                            ...
                            formatLabel={(value, index) => '2018-09-20 03:00:00'}
            ...
            //this.state.xAxis[index].split(' ')[0] + this.state.xAxis[index].split(' ')[1]
        )
